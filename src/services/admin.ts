...
export const adminService = {
  ...
  async approvePaymentRequest(requestId: string, approval: PaymentApprovalRequest): Promise<void> {
    await adminApi.post(`/payments/${requestId}/approve`, approval);
  },

  async rejectPaymentRequest(requestId: string, approval: PaymentApprovalRequest): Promise<void> {
  ...
  // Course management
  async getCourses(page: number = 1, limit: number = 20, search?: string, category?: string, status?: string): Promise<CoursesResponse> {
    const params = new URLSearchParams({
      page: page.toString(),
      limit: limit.toString(),
    });
  ...
  async updateCourseStatus(courseId: string, isActive: boolean): Promise<void> {
    await adminApi.put(`/courses/${courseId}/status`, { is_active: isActive });
  },
  ...
  async getEnrollmentAnalytics(period: string = '30d'): Promise<any> {
    const response = await adminApi.get(`/analytics/enrollments?period=${period}`);
    return response.data;
  },
  ...
  async getFinancialReport(startDate?: string, endDate?: string): Promise<any> {
    ...
    const response = await adminApi.get(`/reports/financial?${params.toString()}`);
    return response.data;
  },
  ...
  async getUserReport(startDate?: string, endDate?: string): Promise<any> {
    ...
    const response = await adminApi.get(`/reports/users?${params.toString()}`);
    return response.data;
  },
  ...
  async getCourseReport(startDate?: string, endDate?: string): Promise<any> {
    ...
    const response = await adminApi.get(`/reports/courses?${params.toString()}`);
    return response.data;
  },
};
