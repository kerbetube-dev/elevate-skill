...
class PaymentService {
  ...
  async getActivePaymentAccounts(): Promise<PaymentAccount[]> {
    try {
      const response = await api.get(`${API_BASE_URL}/payment-accounts/?active_only=true`);
  ...
  async getPaymentAccountById(accountId: string): Promise<PaymentAccount> {
    try {
      const response = await api.get(`${API_BASE_URL}/payment-accounts/${accountId}`);
      return response.data;
    } catch (error: any) {
      console.error('Error fetching payment account:', error);
      throw new Error(error.response?.data?.detail || 'Failed to fetch payment account');
    }
  }
   ...
   */
  async uploadTransactionScreenshot(file: File): Promise<{ url: string; filename: string; message: string }> {
    try {
      const formData = new FormData();
      formData.append('file', file);

      const response = await api.post(`${API_BASE_URL}/payments/upload-screenshot`, formData, {
        headers: {
  ...
  async createPaymentRequest(data: CreatePaymentRequestData): Promise<PaymentRequest> {
    try {
      const response = await api.post(`${API_BASE_URL}/payments/requests`, data);
      return response.data;
    } catch (error: any) {
  ...
  async getMyPaymentRequests(): Promise<PaymentRequest[]> {
    try {
      const response = await api.get(`${API_BASE_URL}/payments/requests/my`);
      return response.data;
    } catch (error: any) {
  ...
  async getPaymentRequestById(requestId: string): Promise<PaymentRequest> {
    try {
      const response = await api.get(`${API_BASE_URL}/payments/requests/${requestId}`);
      return response.data;
    } catch (error: any) {
      console.error('Error fetching payment request:', error);
      throw new Error(error.response?.data?.detail || 'Failed to fetch payment request');
    }
  }
  ...
  async getMyEnrollments(): Promise<Enrollment[]> {
    try {
      const response = await api.get(`${API_BASE_URL}/payments/enrollments/my`);
      return response.data;
    } catch (error: any) {
      console.error('Error fetching enrollments:', error);
      throw new Error(error.response?.data?.detail || 'Failed to fetch enrollments');
    }
  }
  ...
  async checkEnrollment(courseId: string): Promise<boolean> {
    try {
      const response = await api.get(`${API_BASE_URL}/payments/enrollments/${courseId}/check`);
