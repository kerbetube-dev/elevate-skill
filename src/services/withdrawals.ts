...
class WithdrawalsService {
	...
	async getAllWithdrawals(): Promise<WithdrawalResponse[]> {
		try {
			...
			throw new Error(
				error.response?.data?.message ||
					"Failed to fetch all withdrawal requests"
			);
		}
	}
	...
	async approveWithdrawal(
		...
		try {
			const url = admin_notes
				? `${API_BASE_URL}/withdrawals/${withdrawalId}/approve?admin_notes=${encodeURIComponent(
						admin_notes
				  )}`
				: `${API_BASE_URL}/withdrawals/${withdrawalId}/approve`;

			await axios.post(url, {}, { headers: this.getAdminAuthHeaders() });
		} catch (error: any) {
			throw new Error(
				error.response?.data?.message ||
	...
	async rejectWithdrawal(
		...
		try {
			await axios.post(
				`${API_BASE_URL}/withdrawals/${withdrawalId}/reject?rejection_reason=${encodeURIComponent(
					rejection_reason
				)}`,
				{},
				{ headers: this.getAdminAuthHeaders() }
